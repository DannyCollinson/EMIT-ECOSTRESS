...
# modules_path = 'VITALS\\user_contributed\\modules'
# if os.path.join(project_path, modules_path) not in sys.path:
    # sys.path.append(os.path.join(project_path, modules_path))
# from emit_tools import emit_xarray
...
#     os.path.join(
#         project_path,
#         'Data\\Clean_Data\\emit_002.nc',
#     )
# )
# emit_003_clean_path = (
#     os.path.join(
#         project_path,
#         'Data\\Clean_Data\\emit_003.nc',
#     )
# )
...
#     os.path.join(
#         project_path,
#         'Data\\Matched_Data\\eco_on_emit_xy.nc'
#     )
# )
...
# %%
eco_unique = eco_df.reset_index().drop_duplicates(subset=['x', 'y'])
eco = eco_unique.set_index(['x', 'y']).to_xarray()
eco = eco.assign_attrs(eco_slt.attrs)
eco = eco.reset_coords('spatial_ref', drop=True)
eco = eco.assign_coords({'spatial_ref':eco_slt_xr.variables['spatial_ref']})
...
# %%
emit_x = np.empty(len(emit_lat)) + np.nan
emit_y = np.empty(len(emit_lon)) + np.nan

for x_ind, lat_val in enumerate(emit_lat):
    emit_x[x_ind] = transform.TransformPoint(
        lat_val,
        emit_lon[int(x_ind / 0.81)],
    )[0]
    
for y_ind, lon_val in enumerate(emit_lon):
    emit_y[y_ind] = transform.TransformPoint(
        emit_lat[int(y_ind * 0.8)],
        lon_val,
    )[1]
...
for i in range(len(x)):
    for j in range(len(y)):
        ref[x_order[i], y_order[j], :] = ref_old[i, j, :]
        elev[x_order[i], y_order[j]] = elev_old[i, j]

data_vars = {
    'reflectance':(('x', 'y', 'wavelengths'), ref),
    'elev':(('x', 'y'), elev),
}

# %%
emit_xy = xr.Dataset(data_vars, coords)
emit_xy = emit_xy.assign_attrs(emit.attrs)

# %%
emit_xy.to_netcdf(emit_xy_path)

# %%
interp_eco = eco.interp_like(emit_xy, assume_sorted=True)
...
# %%
emit_xy = xr.open_dataset(emit_xy_path)
eco_on_emit = xr.open_dataset(eco_on_emit_path)

# %%
data_xy = emit_xy.assign({'LSTE': eco_on_emit['LSTE']})

# %%
lste = data_xy['LSTE'].values
ref_sum = data_xy['reflectance'].values.sum(axis=2)
elev = data_xy['elev'].values

# %%
nan_mask = np.isnan(lste) + np.isnan(ref_sum) + np.isnan(elev)
nan_mask = ~nan_mask.astype(bool)
data_xy = data_xy.assign_coords({'good_coords': (('x', 'y'), nan_mask)})

# %%
data_xy.to_netcdf(data_xy_path)

# %%
data_xy = xr.open_dataset(data_xy_path)

# %%
good_coords = data_xy['good_coords'].values

ref = data_xy['reflectance'].values
ref_good = ref[good_coords, :]
elev = data_xy['elev'].values
elev_good = elev[good_coords]
lste = data_xy['LSTE'].values
